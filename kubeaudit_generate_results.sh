...
# This script takes as input a chart name and runs all security tools on it, starting always with Kubeaudit.
...
kubeaudit all -f templates/${chart_folder}_template.yaml --minseverity "error" --format json > results_${iteration}.json
...
echo "Step 3 - Debug"
export iteration="2"
kubeaudit all -f fixed_templates/${chart_name}_kubeaudit_fixed_template.yaml --minseverity "error" -c kubeaudit-config.yaml
