...
# This script takes as input a chart name and runs all security tools on it, starting always with Kics.
...
# Step 1 - KICS
echo -e "\n -------------------------- \n"
echo "Step 1 - Run KICS"
kics scan -p templates/${chart_folder}_template.yaml --exclude-severities info --disable-secrets -o . > /dev/null 2>&1
mv results.json results_${iteration}.json
python .github/scripts/main.py --count-checks

# Step 2 - Fix KICS output
echo -e "\n -------------------------- \n"
...
echo "Step 3 - Debug"
export iteration="2"
kics scan -p fixed_templates/${chart_name}_kics_fixed_template.yaml --exclude-severities info --disable-secrets --exclude-queries bb241e61-77c3-4b97-9575-c0f8a1e008d0 --exclude-queries 7c81d34c-8e5a-402b-9798-9f442630e678
